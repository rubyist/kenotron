# This is based on code by Jason Peebles (https://github.com/JasonPeebles/KenKen)
# It has been modified slightly to be compatible with MacRuby/Rubymotion
...
module KenKen
	...
	class Cage	
  	...
  	#Example: "3+ 1 2"
  	CagePattern = "([1-9]\\d*)([\\+\\-\\*\\/\\.])(( \\d*)+)(\\s\\|\\s*)".freeze
  ...
  class Puzzle
    PuzzlePattern = "([1-9]\\d*) ((#{Cage::CagePattern})+)".freeze
    ...
    def initialize(line)
      ...
      cageMatch = cageRegExp.match(cageSpecifiers)

      while cageMatch do
        @cages << Cage.new(cageMatch.string, @grid_side_length)

        cageMatch = Regexp.new(Cage::CagePattern).match(cageMatch.post_match)
      end
